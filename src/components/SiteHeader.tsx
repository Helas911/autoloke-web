        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          {loading ? null : user ? (
            <>
              ...
              >
                {user.photoURL ? <img src={user.photoURL} alt="" className="h-6 w-6 rounded-full" /> : <span className="grid h-6 w-6 place-items-center rounded-full bg-white/10 text-xs">👤</span>}
                <span className="hidden max-w-[160px] truncate sm:inline">{user.displayName || user.email || t(country, "myAccount")}</span>
              </Link>
              <button onClick={onLogout} className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm font-extrabold text-white/90 hover:bg-white/10">
                {t(country, "logout")}
              </button>
            </>
          ) : (
            <>
              <Link href="/prisijungti" className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm font-extrabold text-white/90 hover:bg-white/10">
                {t(country, "login")}
              </Link>
