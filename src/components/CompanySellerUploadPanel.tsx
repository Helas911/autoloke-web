    ...
    <div className="mx-auto mt-4 w-full max-w-6xl px-4">
      <div className="rounded-[24px] border border-white/10 bg-white/[0.03] p-4">
        <label className="flex cursor-pointer items-start gap-3">
          ...
          <span>
            <span className="block text-sm font-black text-white">{isDk ? "Jeg er virksomhed / forhandler" : "Esu įmonė / pardavėjas"}</span>
            <span className="mt-1 block text-xs font-semibold text-white/55">{isDk ? "Valgfrit. Private sælgere behøver ikke udfylde dette afsnit." : "Nebūtina. Paprastiems žmonėms šios skilties pildyti nereikia."}</span>
          </span>
        </label>

        {profile.enabled ? (
          <div className="mt-4 grid gap-2 md:grid-cols-2">
            <input value={profile.sellerName} onChange={(e) => updateProfile({ sellerName: e.target.value })} placeholder={isDk ? "Virksomhedsnavn" : "Įmonės pavadinimas"} className="w-full rounded-2xl border border-white/12 bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-white/45" />
            <input value={profile.sellerWebsite} onChange={(e) => updateProfile({ sellerWebsite: e.target.value })} placeholder={isDk ? "Hjemmeside (valgfrit)" : "Internetinė svetainė (nebūtina)"} className="w-full rounded-2xl border border-white/12 bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-white/45" />
            <input value={profile.sellerWorkingHours} onChange={(e) => updateProfile({ sellerWorkingHours: e.target.value })} placeholder={isDk ? "Åbningstider (valgfrit)" : "Darbo laikas (nebūtina)"} className="w-full rounded-2xl border border-white/12 bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-white/45" />
            <input value={profile.sellerLogoUrl} onChange={(e) => updateProfile({ sellerLogoUrl: e.target.value })} placeholder={isDk ? "Logo-URL (valgfrit)" : "Logotipo nuoroda (nebūtina)"} className="w-full rounded-2xl border border-white/12 bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-white/45" />
            <input value={profile.sellerCoverUrl} onChange={(e) => updateProfile({ sellerCoverUrl: e.target.value })} placeholder={isDk ? "Coverbillede-URL (valgfrit)" : "Viršelio nuotraukos nuoroda (nebūtina)"} className="w-full rounded-2xl border border-white/12 bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-white/45 md:col-span-2" />
            <div className="flex flex-wrap items-center gap-2 md:col-span-2">
              <button type="button" onClick={saveProfile} className="rounded-2xl bg-white px-4 py-3 text-sm font-black text-black hover:bg-white/90">{isDk ? "Gem profil" : "Išsaugoti profilį"}</button>
              {saved ? <span className="text-xs font-bold text-emerald-200">{isDk ? "Gemt ✅" : "Išsaugota ✅"}</span> : null}
              {applied ? <span className="text-xs font-bold text-blue-200">{isDk ? "Profilen er knyttet til den nye annonce ✅" : "Profilis priskirtas naujam skelbimui ✅"}</span> : null}
            </div>
          </div>
        ) : null}
      </div>
